...
def main():
    ...
    dataset = loadtxt('../data/stats/shots_2013_2014.tsv', delimiter='\t', dtype='f8', skiprows=1)
    target = [x[0] for x in dataset]
    train = [x[1:] for x in dataset]
    test = genfromtxt(open('../data/stats/shots_2013_2014.tsv','r'), delimiter='\t', dtype='f8', skip_header=1)
    ...
    savetxt('../data/stats/exp_goals_v2.tsv', results, delimiter='\t', fmt='%f', header="x\ty\thead\tpredict")
