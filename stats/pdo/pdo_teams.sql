SELECT t.short_name, 
       ROUND((SUM(gae_ht.goals)/SUM(gae_ht.on_target) + 1-SUM(gae_at.goals)/SUM(gae_at.on_target)) * 1000)::numeric AS pdo_int, 
       REPLACE(ROUND((SUM(gae_ht.goals)/SUM(gae_ht.on_target) + 1-SUM(gae_at.goals)/SUM(gae_at.on_target)) * 1000)::text, ',', '') AS pdo
FROM
(
